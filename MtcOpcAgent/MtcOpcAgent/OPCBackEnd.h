...
HRESULT COPCBackEnd<TOPCItem>::Connect() 
{
	...
	DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s \n",_opcservermachine.c_str() ));
	...
	try 
	{
		...
		DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - TestPing %d\n",_opcservermachine.c_str(),_nPingTimeout ));
		if(_nPingTimeout >0 && Ping(_opcservermachine.c_str(), _nPingTimeout))
		{
			::Sleep(20*1000);
		...
		{
			DebugMessage(StdStringFormat("OPCBackEnd::Connect Machine %s - Local CoCreateInstance\n",_opcservermachine.c_str() ));
			...
			{
				_pIOPCServer=NULL;
				DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - Couldn't create CoCreateInstance\n",_opcservermachine.c_str() ));
			...
			DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - Remote CoCreateInstanceEx\n",_opcservermachine.c_str() ));
			...
			if (FAILED(hResult))
			{
				DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - CoCreateInstanceEx hResult Error %x\n",_opcservermachine.c_str(), hResult ));
				throw StdStringFormat (_T("FAIL: Connect() Remote CoCreateInstanceEx.\n"));
			}

			if (FAILED(mqi[0].hr)) 
			{
				DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - CoCreateInstanceEx mqi[0].hr Error %x\n",_opcservermachine.c_str(), mqi[0].hr ));
		...
		if(_pIOPCServer==NULL)
		{
			DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - CoCreateInstanceEx pointer NULL error %x\n",_opcservermachine.c_str() ));
			return E_FAIL;
		}

		// This is how we will detect if the server is connected to the CNC

		DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - GetStatus\n",_opcservermachine.c_str() ));
		...
		DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - AddGroup\n",_opcservermachine.c_str() ));
		...
		{
			_pIOPCItemMgt=NULL;
			DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - AddGroup couldn't add group to server %x\n",_opcservermachine.c_str(),hResult ));
			throw std::string (_T("FAIL: Connect() couldn't add group to server!\n"));
		}


		// Get interface for read operations
		DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - QueryInterface IID_IOPCSyncIO\n",_opcservermachine.c_str() ));
		if ( FAILED(hResult =  _pIOPCItemMgt->QueryInterface(IID_IOPCSyncIO,(void**)&_pIOPCSyncIO )))
		{
			_pIOPCSyncIO=NULL;
			DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - couldn't cculdn't query IOPCSyncIO interface %x\n",_opcservermachine.c_str(),hResult ));
		...
		// Add items to group
		/////////////////////////////////////////////////////////////////////////////
		DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - Add items to group\n",_opcservermachine.c_str() ));
		...
		for(j=0,i=0; i< opcitems.size(); i++)
		{
			...
			if(opcitems[i]->vt== VT_USERDEFINED)
				continue;
			DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - Add item %s\n",_opcservermachine.c_str(),opcitems[i]->_tagname.c_str() ));
		...
		if((j==0) && (opcitems.size()>0))
		{
			...
			DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - Add item error %s\n",_opcservermachine.c_str(), err.c_str() ));
		}

	}
	catch(std::string errmsg)
	{
		std::string hrerr=StdStringFormat(_T("FAIL: Connect() OPC Server %s Connect failed %s COM Error=%x \n"),  _opcservermachine.c_str(), errmsg.c_str(), hResult);
		ErrorMessage( hrerr);
	...
	DebugMessage(StdStringFormat("COPCBackEnd::Connect Machine %s - Exit\n",_opcservermachine.c_str()));
	return S_OK;
}

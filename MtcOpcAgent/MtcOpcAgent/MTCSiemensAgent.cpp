...
void AgentConfigurationEx::initialize(int aArgc, const char *aArgv[])
{
	...
	try{
		std::string path(::ExeDirectory()+"MtcOpcAgent.ini");
		std::string cfgfile = ::ExeDirectory()+"MtcOpcAgent.ini"; // path.substr(0,path.find_last_of(".")) + ".ini";
		...
		if(GetFileAttributesA(cfgfile.c_str())!= INVALID_FILE_ATTRIBUTES)
		{
			...
			MTConnectService::setName(config.GetSymbolValue("Agent.ServiceName", "MtcOpcAgent").c_str());

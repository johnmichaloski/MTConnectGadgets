	...
	try 
	{
		...
		for(int i=0; i< xml.resources.size(); i++)
		{
			...
			if(!xml.resources[i].mtbf.IsEmpty() && !xml.resources[i].mttr.IsEmpty())
			{
				...
				server->PutData ( L"FailUp(1)", xml.resources[i].mtbf.MapArenaDistribution());				
				server->PutData ( L"FailDown(1)", xml.resources[i].mttr.MapArenaDistribution());				
			}
			// FIXME: add downtimes
		}
		...
		{
			// First check if 
			if(xml.cells[i].resourceIds.size() == 1)
				continue;
			CComPtr<Arena::IModule> set=_Modules->Create(_bstr_t(L"BasicProcess"), _bstr_t(L"Set"), 00, 0);
			set->PutData ( _bstr_t("Name"), _bstr_t(xml.cells[i].name+"ResourceSet"));
			set->PutData ( _bstr_t("Type"), _bstr_t(L"Resource"));
			for(int j=0; j< xml.cells[i].resourceIds.size(); j++)
			{
				Resource r = xml.FindResourceById(xml.cells[i].resourceIds[j]);
		...
		for(int i=0; i< xml.cells.size(); i++)
		{
			...
			if(xml.cells[i].resourceIds.size() == 1)
			{
				Resource r = xml.FindResourceById(xml.cells[i].resourceIds[0]);
				server=_Modules->Create(_bstr_t(L"Common"), _bstr_t(L"Server"), 0, i*700);
				server->PutData ( _bstr_t("Label"), xml.cells[i].name);
				server->PutData ( _bstr_t("Station"), xml.cells[i].name);
			...
			else // Define Cells with AdvServer containing resources > 1
			{
				...
				server->PutData ( _bstr_t("Station"), xml.cells[i].name);
				server->PutData ( _bstr_t("ResSG"), _bstr_t(L"Resource Set"));
				server->PutData ( _bstr_t("Set"), xml.cells[i].name+"ResourceSet");
		...
		for(int i=0; i<xml.parts.size(); i++)
		{
			...
			set->PutData ( _bstr_t("Other(" + n + ")"), xml.parts[i].processplanidentifier); //  _bstr_t(L"Part" + n + "ProcessPlan"));
		}

		for(int i=0; i< xml.processplans.size(); i++)
		{
			ProcessPlan plan = xml.processplans[i];
			CComPtr<Arena::IModule> var=_Modules->Create(_bstr_t(L"AdvancedTransfer"), _bstr_t(L"Sequence"), 00, 0);

			// Assume type is sequence
			Process seq = plan.FindProcess(plan.firstProcessId);
			// Sequence
			//Part1ProcessPlan
			var->PutData ( _bstr_t("Name"), xml.processplans[i].identifier);
			for(int m=0; m < seq.steps.size(); m++)
			{
				// THis is the processplan process name, map into cell name
				Process step = plan.FindProcess(seq.steps[m]);
				bstr_t station;

				if(step.resourcesRequired.size() < 1)
					continue;
				Cell cell  =xml.FindCellById(step.resourcesRequired[0]); 
				_bstr_t timings[] = { _bstr_t(L"TRIA(6,8,10)"),_bstr_t("TRIA(5,8,10)"),_bstr_t("TRIA(5,8,10)"),_bstr_t("TRIA(15,20,25)")};
			...
			bstr_t n = StdStringFormat("%d",seq.steps.size()+1).c_str();
...
void CComArenaHook::JobShopBuilder()
{
	HRESULT hr;
	try 
	{
		...
		if(xml.jobs.size() > 0)
		{
			...
			for(int k=0; k<xml.jobs[0].partIds.size(); k++)
			{
				//	for(int i=0; i<xml.parts.size(); i++)
				//{
				//Part part = xml.parts[i];
				Part part = xml.FindPartById(xml.jobs[0].partIds[k]);
				// Create Parts
				...
				create->PutData ( _bstr_t(L"Max Batches"), xml.jobs[0].partQuantity[k]);  // FIXME: part of order?
		...
		for(int i=0; i< xml.resources.size(); i++)
		{
			...
			resource->PutData ( _bstr_t("Name"), xml.resources[i].name);
		...
		for(int i=0; i< xml.cells.size(); i++)
		{
			...
			station->PutData ( _bstr_t(L"Name"), xml.cells[i].name);
			station->PutData ( _bstr_t(L"Statn"), xml.cells[i].name);
			...
			else
			{
				...
				process->PutData ( _bstr_t("Set Name(1)"), xml.cells[i].name+"ResourceSet");
				process->PutData ( _bstr_t(L"Quantity(1)"), _bstr_t("1"));
			}
		...
		variable->PutTitleText("Shifts to complete work");
		variable->PutTitlePercentHeight(50);
		for(int k=0; xml.jobs.size() > 0 && k<xml.jobs[0].partIds.size(); k++)
		{
			//	for(int i=0; i<xml.parts.size(); i++)
			//{
			//Part part = xml.parts[i];

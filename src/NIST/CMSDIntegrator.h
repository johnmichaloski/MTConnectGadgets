...
public:
	...
	template<typename T>
	void SaveProperties(T &_parent, std::vector<bstr_t> &properties, std::vector<bstr_t> & distributions)
	{
		...
		{	
			CMSD::CProperty prop = _parent.Property.append();
			Distribution *dist = CCMSDIntegrator::FindDistributionById(distributions[i]);
			dist->Save(prop);
		}
	...
	template<typename T>
	void LoadProperties(T &_parent, std::vector<bstr_t> &properties,  std::vector<bstr_t> &distributions)
	{
		...
		for(int i=0; i < _parent.Property.count();i++)
		{
			...
			if(prop.Distribution.count() > 0)
			{
				Distribution * astat ( (Distribution *) IObject::CreateSave<Distribution>() );
				//astat->LoadProperty(prop.GetNode());
				astat->LoadDistribution(prop.Distribution[0]);
				distributions.push_back(astat->identifier);
				continue; // don't need to add as property now - FIXME
			}
...
struct Documentation : public IObject
{
	Documentation();
	void Load(MSXML2::IXMLDOMNodePtr  ini);
	void Save(CMSD::CCMSDDocumentReference &doc);
...
{
	CostAllocation();
	void Load(MSXML2::IXMLDOMNodePtr  cost);
	void Save(CMSD::CCostAllocationData &cost);
	bstr_t name;
	bstr_t description;
	...
	BEGIN_METADATA(CostAllocation)
		...
		STDFLD(category, type_is_bstr_t),
		STDFLD(currencyType, type_is_bstr_t),
		STDFLD(properties, type_is_propertylist),
		END_METADATA
};
...
class CCMSDAsset : public CCMSDIntegrator
{
public:
	CCMSDAsset::CCMSDAsset(void)
	{
		CCMSDIntegrator::resources=  &_resources;
		CCMSDIntegrator::cells=&_cells;
		CCMSDIntegrator::parts=&_parts;
		CCMSDIntegrator::processplans=&_processplans;
		CCMSDIntegrator::jobs=&_jobs;
		CCMSDIntegrator::distributions=&_distributions;
		CCMSDIntegrator::calendars=&_calendars;
		CCMSDIntegrator::layouts=&_layouts;
		CCMSDIntegrator::costs=&_costs;
		CCMSDIntegrator::documentation=&_documentation;
	}
	std::string GenerateSinglePartJobXml(CCMSDIntegrator * cmsd, std::string partid);
	void WriteToAgent(std::string, std::string _device, AgentConfiguration * _agent);
	void Clear()
	{
		_resources.clear();
		_cells.clear();
	...
	std::vector<IObjectPtr> _resources;
	std::vector<IObjectPtr> _cells;
	std::vector<IObjectPtr> _parts;
	std::vector<IObjectPtr> _processplans;
	std::vector<IObjectPtr> _jobs;
	std::vector<IObjectPtr> _distributions;
	std::vector<IObjectPtr> _calendars;
	std::vector<IObjectPtr> _layouts;
	std::vector<IObjectPtr> _costs;
	std::vector<IObjectPtr> _documentation;

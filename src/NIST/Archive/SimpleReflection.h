...
public:
	...
	virtual std::vector<IObjectPtr> & objects() { static  std::vector<IObjectPtr>  _objects; return _objects; }
	...
	template<typename X>
	static X * CreateSave() {X * x = new X(); x->objects().push_back((IObjectPtr) x); return x;  }

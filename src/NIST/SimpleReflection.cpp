...
std::string IObject::GetProperty(int index, int field)
{

	if(index < 0 || index >= properties.size())
		return "";

	std::vector<std::string> tokens = Tokenize(RightTrim(std::string((LPCSTR) properties[index]),";"),",");
	if(field < 0 || field >= tokens.size())
		return "";

	return tokens[field];
}

std::string IObject::GetPropertyName(std::string propertystr)
{
	std::vector<std::string> tokens = Tokenize(RightTrim(propertystr,";"),",");
	if(tokens.size() > 0)
		return tokens[0];
	return "";
...
std::string IObject::GetPropertyUnits(std::string name)
{
	for(int i=0; i< properties.size(); i++)
	{
		std::vector<std::string> tokens = Tokenize(RightTrim(std::string((LPCSTR) properties[i]),";"),",");
		if(tokens.size() < 2)
			continue;
		if(tokens[0]==name)
			if(tokens.size() > 2) return tokens[2];
	}
	return "";
}

std::string IObject::GetPropertyDescription(std::string name)
{
	for(int i=0; i< properties.size(); i++)
	{
		std::vector<std::string> tokens = Tokenize(RightTrim(std::string((LPCSTR) properties[i]),";"),",");
		if(tokens.size() < 2)
			continue;
		if(tokens[0]==name)
			if(tokens.size() > 3) return tokens[3];
	}
	return "";
}

//
// ShdrCommandCellView.cpp : implementation of the CShdrCommandCellView class
//
...
BOOL CShdrCommandCellView::PreTranslateMessage(MSG* pMsg)
{
...
LRESULT CShdrCommandCellView::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	_devicename =this->GetDlgItem(IDC_DEVICENAME);
	_devicetype=this->GetDlgItem(IDC_DEVICENAME2);
	_socketnumber=this->GetDlgItem(IDC_DEVICENAME3);
	
	return 0;
}

void CShdrCommandCellView::Init(std::string devicesxmlfilename, 
								int ipport, 
								std::string devicename)
{
	_shdr.Init(devicesxmlfilename, ipport, devicename);
	_devicename.SetWindowText(("Name = " + devicename).c_str());
	_devicetype.SetWindowText(("Type = " + ExtractFilename(devicesxmlfilename)).c_str());
	_socketnumber.SetWindowText(StdStringFormat("Port = %d",ipport).c_str());
}

void CShdrCommandCellView::FillRow(int nNewItem, MTConnectDataModel::iterator it )
{
	// Row i
...
void CShdrCommandCellView::InitBasicList()
{
	LOGFONT logFont = { 0 };
...
void CShdrCommandCellView::AddLastRow()
{		
	int nNewItem = m_wndListCtrl.AddItem("");
	m_wndListCtrl.SetItemFormat(nNewItem,0,  ITEM_FORMAT_ADDBOX,ITEM_IMAGE_ADDBOX );
...
void CShdrCommandCellView::Update()
{
	int i;
...
LRESULT CShdrCommandCellView::OnBnClickedButton2(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	Update();
	return 0;
}

LRESULT CShdrCommandCellView::OnDoubleClicked( LPNMHDR lpNMHDR )
{
...
int CShdrCommandCellView::GetRowNumber(std::string item)
{
...
LRESULT CShdrCommandCellView::OnBnClickedButton1(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	i = GetRowNumber("power");
	if(i>0)	m_wndListCtrl.SetItemText( i, VALUECOL,"ON");
	i = GetRowNumber("avail");
	if(i>0)	m_wndListCtrl.SetItemText( i, VALUECOL,"AVAILABLE");
	//i = GetRowNumber("Xabs");
	//if(i>0)	m_wndListCtrl.SetItemText( i, VALUECOL,"AVAILABLE");

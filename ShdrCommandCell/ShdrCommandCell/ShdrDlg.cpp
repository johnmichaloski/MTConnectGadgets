// ShdrDlg.cpp : implementation of the CShdrDlg class
//
/////////////////////////////////////////////////////////////////////////////
...
int CShdrDlg::m_nMachines=1;
int CShdrDlg::Port0 = 7878;
...
void CShdrDlg::InsertListItem(int n)
{
	m_wndListCtrl.InsertItem(n, "");
	FillRow(n);
}
void CShdrDlg::DeleteListItem(int nSelectedRow)
{
	m_wndListCtrl.DeleteItem( nSelectedRow );
}

std::string CShdrDlg::FindConfigFile(std::string name)
{
	std::string contents;
	ReplaceAll(name, ".", "\\");
...
LRESULT CShdrDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	if ( !m_ilItemImages.CreateFromImage( IDB_EXAMPLE, 16, 0, RGB( 255, 0, 255 ), IMAGE_BITMAP, LR_CREATEDIBSECTION ) )
		return FALSE;
	m_wndListCtrl.SubclassWindow( GetDlgItem( IDC_LISTCTRL2 ) );
	DlgResize_Init(true);
...
void CShdrDlg::UpdateLists(std::string config)
{
	Clear();
...
void CShdrDlg::UpdateLists()
{
	...
	for(i=0; i< Tablesize() -1 ; i++)
	{
		...
		devicefiles.push_back(FindConfigFile((LPCSTR) tmp));
...
LRESULT CShdrDlg::OnNewCfg(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
...
LRESULT CShdrDlg::OnOK(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& bHandled)
{
	UpdateLists();
	Port0 += devices.size(); // rough guess to port numbering
	EndDialog(wID);
	return 0;
}

LRESULT CShdrDlg::OnCancel(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	EndDialog(wID);
	return 0;
}
...
void CShdrDlg::FillRow(int nNewItem )
{
	FillRow(nNewItem, 
		StdStringFormat("M%03d", m_nMachines++ ),
		"127.0.0.1",
		StdStringFormat("Generic", nNewItem ),
		StdStringFormat("%d", Port0 + nNewItem  )
		);
}
void CShdrDlg::FillRow(int nNewItem, 
...
LRESULT CShdrDlg::OnDoubleClicked( LPNMHDR lpNMHDR )
{
...
LRESULT CShdrDlg::OnClicked( LPNMHDR lpNMHDR )
{
	CListNotify *pListNotify = reinterpret_cast<CListNotify *>( lpNMHDR );
...
LRESULT CShdrDlg::OnClick(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& /*bHandled*/)
{ 
	CButton c(hWndCtl);
	...
	for(int i=0; i<  Tablesize() ; i++)
	{
		...
		{
			device="CNC1";
			shdrport =StdStringFormat("%d", Port0+i);
		}
		else
		{

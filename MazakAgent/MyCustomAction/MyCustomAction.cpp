...
extern "C" UINT __stdcall Install(MSIHANDLE hInstall)
{
	...
	try 
	{
		...
		RunSynchronousProcess("sc.exe", " stop MazakAgent");
		RunSynchronousProcess("sc.exe", " delete MazakAgent");
		...
		for(int i=0 ; i< symbols.size() ; i++)
		{
			...
			if(tokens[0]=="HttpPort") // no change necessary if using csv, program will stip out
				httpPort=tokens[1];    
			if(tokens[0]=="User") 
				user=tokens[1]+"MazakAgent\\";    
		}
		//path="C:\\MTConnect\\MazakAgent\\";
		// \"C:\\Program Files\\MTConnect\\MazakAgent\\ \   
		...
		status="Read MazakAgent.ini";
		std::string contents; 
		ReadFile(path+"MazakAgent.ini", contents);

		//GrantFullAccess (hInstall, (path+"MazakAgent.ini").c_str()) ;
		...
		WriteFile(path+"MazakAgent.ini",contents);
		...
		status="Create service with MazakAgent.exe";
		RunSynchronousProcess(path+"MazakAgent.exe", " install");
		status="sc Start service MazakAgent";
...
{
	std::string errmsg =  StdStringFormat("MazakAgent Install In trans_func - Code = 0x%x\n",  pExp->ExceptionRecord->ExceptionCode);

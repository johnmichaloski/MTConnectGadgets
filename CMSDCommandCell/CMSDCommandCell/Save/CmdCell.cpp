...
void CJobCommands::Update(CResourceHandlers * resourceHandlers)
{
	for(int i=0; i < size() ; i++)
	{
		...
		if(	resource->outqueue.CanPop())
		{
			...
			if(_current>= size())
				break;	
			resource = resourceHandlers->FindResourceHandler(_current) ; // _resourceHandlers, _current,at(i)->_ResourceRequired[_current]);
			if(resource == NULL) 
				continue;
		}
		...
		{
			int nCurrentStep =at(i)->_currentstep;
			if(nCurrentStep< 0 || nCurrentStep>= at(i)->MaxStep()) 
			{
				DebugBreak();
			}
...
void CJobCommands::process(CJobCommands * jobs, CResourceHandlers *  resourceHandlers)
{
	...
	while(m_bRunning)
	{
		...
		if(AllFinished())
			break;
		_dUpdateRateSec +=  1.0 * ControlThread::_dSpeedup;
		...
		for(int i=0 ; i<_resourceHandlers->size(); i++)
			dUpdateWait=MAX(1.0, _resourceHandlers->at(i)->GetUpdateFactor());
		OutputDebugString("UpdateWait=");OutputDebugString(ConvertToString(dUpdateWait).c_str()); 
	...
	std::string header = "Machine," + _resourceHandlers->at(0)->GenerateCSVHeader() + ",Utility<BR>Costs</BR>,Description";
	...
	{std::string html1=_resourceHandlers->at(i)->Name() + ",";
		html1+= _resourceHandlers->at(i)->GenerateCSVTiming() + "," ;
